...
ENTRY = {
    ...
    "source": {
        "source": "local",
        "path": "./plugins/yandex-direct-agent",
    },
    "policy": {
...
def _load_marketplace(path: Path) -> dict:
    ...
    if not isinstance(data.get("interface"), dict):
        data["interface"] = {"displayName": "Repo Plugins"}

    if not data.get("name"):
        data["name"] = "repo-marketplace"
...
def main() -> int:
    ...
    print(f"[OK] Repo plugin copied to {target_plugin_dir}")
    print(f"[OK] Marketplace entry upserted in {marketplace_path}")
    print("Next steps:")
